...
export function useKey(keys: string, callback: UseKeyCallback, options: UseKeyOptions = {}) {
  ...
  const handler: UseKeyCallback = (event, hotkeysEvent) => {
    if (!unref(options.input) && isInputing()) return;
    if (unref(options.prevent)) event.preventDefault();
    if (unref(options.stop)) event.stopPropagation();
    if (!unref(options.repeat) && event.repeat) return;
  ...
  const init = () => {
    ...
    for (const key of keyList) {
      if (bindings.has(key)) bindings.set(key, [...bindings.get(key)!, handler]);
      else {
